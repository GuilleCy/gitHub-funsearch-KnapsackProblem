def heuristic(items_state):
    ...
    for i in range(num_items):
        ...
        item_properties[i] = (density, values[i], weights[i]) # Properly store for O(1) lookup later
    ...
    while improvement_made:
        ...
        for unsel_idx in list(unselected_items_dict.keys()):
            # Retrieve from item_properties for consistency and efficiency
            unsel_density, unsel_value, unsel_weight = item_properties[unsel_idx] 
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            ...
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                # This check is crucial because current_selected_items_indices might have changed
                # due to earlier phase (0-weight) continue restarting the loop.
        ...
        for sel_value, sel_weight, sel_idx in temp_selected_item_info:
            # This check is crucial because current_selected_items_indices might have changed
            # due to earlier phase (0-weight or 1-for-1) continue restarting the loop.
            if sel_idx not in current_selected_items_indices:
                continue
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density:
                # Optimized: No need to check unsel_idx_to_add in current_selected_items_indices or unsel_idx_to_add == sel_idx
                # as temp_unselected_items_list_by_density is built from unselected_items_dict,
                # and sel_idx is from current_selected_items_indices, which are disjoint sets at this point.
        ...
        for i in range(len(temp_selected_item_info)):
            ...
            # This check is crucial as current_selected_items_indices might have changed
            ...
            for j in range(i + 1, len(temp_selected_item_info)):
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
                    ...
                    # Optimized: No need for checks like unsel_idx in current_selected_items_indices or unsel_idx == sel1_idx or unsel_idx == sel2_idx
                    # because temp_unselected_items_list_by_value contains only unselected items,
                    # and sel1_idx/sel2_idx are currently selected items, thus disjoint.
    ...
    # --- Final Greedy Fill Pass (after local search stabilizes) ---
    # Attempt to fill any remaining capacity with the most dense unselected items
    # without removing any already selected items.
    
    # Re-create and sort the list of unselected items based on their current state.
    # The unselected_items_dict is the authoritative source for currently unselected items.
    final_fill_candidates = []
    for unsel_idx in unselected_items_dict:
        density, value, weight = item_properties[unsel_idx] # Retrieve current properties
        final_fill_candidates.append((density, value, weight, unsel_idx))
    
    final_fill_candidates.sort(key=lambda x: x[0], reverse=True) # Sort by density desc

    for density, value, weight, original_index in final_fill_candidates:
        if current_total_weight + weight <= capacity:
            current_selected_items_indices.add(original_index)
            current_total_weight += weight
            current_total_value += value
            # Note: No need to remove from unselected_items_dict here, as this is the final pass
            # and unselected_items_dict is not used after this for further operations.
