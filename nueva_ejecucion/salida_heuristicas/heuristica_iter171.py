def heuristic(items_state):
    ...
    # --- FINAL GREEDY ADDITION PASS ---
    # Attempt to add any remaining unselected items that fit into the current capacity,
    # without removing any already selected items. This ensures the knapsack is as full
    # as possible with the highest density items that fit, given the current selection.
    
    if unselected_items_dict: # Only proceed if there are unselected items left
        final_add_candidates = []
        for unsel_idx in unselected_items_dict:
            # unselected_items_dict values already store (density, value, weight, original_index)
            final_add_candidates.append(unselected_items_dict[unsel_idx])
        
        # Sort by density in descending order (highest density first), then value descending
        final_add_candidates.sort(key=lambda x: (x[0], x[1]), reverse=True)

        for unsel_density, unsel_value, unsel_weight, unsel_idx in final_add_candidates:
            if current_total_weight + unsel_weight <= capacity:
                current_selected_items_indices.add(unsel_idx)
                current_total_weight += unsel_weight
                current_total_value += unsel_value
                # No need to remove from unselected_items_dict; it's the final pass
                # and unselected_items_dict is not used again for modification.
            # else: continue to next item, as smaller/lighter items might still fit.
            # No break here, as we want to check all items that might fit a small gap.
