def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # Added secondary sort by weight descending to free up more capacity if values are equal.
        selected_for_value_swap_removal = []
        for idx in current_selected_items_indices:
            selected_for_value_swap_removal.append((values[idx], weights[idx], idx))
        selected_for_value_swap_removal.sort(key=lambda x: (x[0], -x[1])) # Value ASC, Weight DESC
        ...
        selected_for_capacity_swap_removal = []
        for idx in current_selected_items_indices:
            selected_for_capacity_swap_removal.append((weights[idx], values[idx], idx))
        selected_for_capacity_swap_removal.sort(key=lambda x: (-x[0], x[1])) # Weight DESC, Value ASC
        ...
        unselected_by_value_desc = list(unselected_items_dict.values())
        unselected_by_value_desc.sort(key=lambda x: (x[1], x[0]), reverse=True) 
        ...
        unselected_by_density_desc = list(unselected_items_dict.values())
        unselected_by_density_desc.sort(key=lambda x: (x[0], x[1]), reverse=True) 
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in unselected_by_value_desc:
            # Optimization: If an unselected item cannot fit even if the knapsack is empty, skip it.
            if unsel_weight > capacity:
                continue
            
            # Try 1-for-1 swap with selected items (lowest value first, then heaviest if values equal)
            for sel_value, sel_weight, sel_idx in selected_for_value_swap_removal:
        ...
        # Prioritize removing heavier items to create more capacity.
        for sel_weight, sel_value, sel_idx in selected_for_capacity_swap_removal: # Note order: weight, value, idx
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in unselected_by_density_desc:
        ...
        # Prioritize removing less valuable items, then heavier ones if values equal.
        for i in range(len(selected_for_value_swap_removal)):
            sel1_value, sel1_weight, sel1_idx = selected_for_value_swap_removal[i]

            for j in range(i + 1, len(selected_for_value_swap_removal)):
                sel2_value, sel2_weight, sel2_idx = selected_for_value_swap_removal[j]
