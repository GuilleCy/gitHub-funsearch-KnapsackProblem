def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # NEW OPTIMIZATION: Selected items for 1-for-k: sorted by weight descending (candidates for removal).
        # This aims to free up maximum capacity for 1-for-k swaps, potentially allowing more new items.
        ...
        for idx in current_selected_items_indices:
            # Store (weight, value, idx) so that the primary sort key is weight
            temp_selected_item_info_by_weight_desc.append((weights[idx], values[idx], idx)) 
        # Sort by weight DESC (-x[0]), then value ASC (x[1]) for tie-breaking
