def heuristic(items_state):
    ...
    while improvement_made:
        ...
        for unsel_idx in list(unselected_items_dict.keys()):
            unsel_density, unsel_value, unsel_weight, _ = unselected_items_dict[unsel_idx] # Retrieve from dict
        ...
        for sel_value, sel_weight, sel_idx in temp_selected_item_info:
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density:
                # No need to check unsel_idx_to_add in current_selected_items_indices or unsel_idx_to_add == sel_idx
                # as temp_unselected_items_list_by_density is built from unselected_items_dict,
                # and sel_idx is from current_selected_items_indices, which are disjoint sets.
        ...
        for i in range(len(temp_selected_item_info)):
            ...
            for j in range(i + 1, len(temp_selected_item_info)):
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
                    ...
                    # No need for checks like unsel_idx in current_selected_items_indices or unsel_idx == sel1_idx or unsel_idx == sel2_idx
