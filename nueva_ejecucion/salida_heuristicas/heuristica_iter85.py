def heuristic(items_state):
    ...
    while improvement_made:
        ...
        for sel_value, sel_weight, sel_idx in temp_selected_item_info:
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density:
                # Optimized check: only need to ensure we don't add the item that was just conceptually removed.
        ...
        for i in range(len(temp_selected_item_info)):
            ...
            for j in range(i + 1, len(temp_selected_item_info)):
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
                    ...
                    # Optimized check: ensure we don't add back items that were just conceptually removed.
    ...
    # --- Final Greedy Fill ---
    # After local search stabilizes, try to add any remaining unselected items that fit.
    # Sort unselected items by density descending to prioritize adding valuable items.
    final_unselected_items_by_density = list(unselected_items_dict.values())
    final_unselected_items_by_density.sort(key=lambda x: x[0], reverse=True)

    for density, value, weight, original_index in final_unselected_items_by_density:
        if current_total_weight + weight <= capacity:
            current_selected_items_indices.add(original_index)
            current_total_weight += weight
            current_total_value += value
