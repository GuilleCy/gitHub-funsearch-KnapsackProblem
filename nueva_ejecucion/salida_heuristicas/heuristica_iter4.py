def heuristic(items_state):
    ...
    # Return numerical value (0.0) as per rule
    ...
    while improvement_made:
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list:
            ...
            # It might have been skipped in the initial greedy pass if capacity was full,
            # but if it was truly unselected, add it now.
            if unsel_weight == 0 and unsel_value > 0:
                current_selected_items_indices.add(unsel_idx)
                current_total_value += unsel_value
                # Remove from unselected_items_dict
            ...
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                ...
                if current_total_weight + weight_change <= capacity and value_change > 0:
                    ...
                    del unselected_items_dict[unsel_idx] # Remove the item that was added to selected
                    # Recalculate density for the item being added back to unselected_items_dict
    ...
    return float(current_total_value) # Return numerical value as per rule
#