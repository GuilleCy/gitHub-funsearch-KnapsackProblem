def heuristic(items_state):
    ...
    while improvement_made:
        improvement_made = False # Reset for current iteration
        ...
        temp_selected_item_info.sort(key=lambda x: x[0]) # Sort by value ascending
        ...
        temp_unselected_items_list_by_value.sort(key=lambda x: x[1], reverse=True) # Sort by value descending
        ...
        if not improvement_made:
            ...
            temp_unselected_items_list_by_density.sort(key=lambda x: x[0], reverse=True) # Sort by density descending
            ...
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                ...
                # Also ensure that at least one item is being added.
