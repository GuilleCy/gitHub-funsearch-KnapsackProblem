def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # --- Phase 1: 0-weight item check & Best 1-for-1 swaps ---
        # First, add any remaining 0-weight items with positive value.
        # This is a 'first improvement' sub-phase because these are always beneficial and don't consume capacity.
        zero_weight_items_to_add = []
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            # Check if it's still unselected (could have been added in a previous outer loop iteration)
            if unsel_idx not in current_selected_items_indices: 
                if unsel_weight == 0 and unsel_value > 0:
                    zero_weight_items_to_add.append((unsel_idx, unsel_value))
        
        if zero_weight_items_to_add:
            for unsel_idx, unsel_value in zero_weight_items_to_add:
                current_selected_items_indices.add(unsel_idx)
                current_total_value += unsel_value
                if unsel_idx in unselected_items_dict: # Should always be true at this point
                    del unselected_items_dict[unsel_idx] 
            improvement_made = True
            continue # Restart the 'while' loop immediately as state has changed

        # Now, find the best 1-for-1 swap
        best_1for1_value_gain = 0
        best_1for1_sel_idx = -1
        best_1for1_unsel_idx = -1
        best_1for1_weight_change = 0

        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            if unsel_idx in current_selected_items_indices: # Item might have been selected by 0-weight logic
                continue
            ...
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                ...
                if current_total_weight + weight_change <= capacity and value_change > best_1for1_value_gain:
                    ...
                    best_1for1_weight_change = weight_change
        ...
        if best_1for1_value_gain > 0:
            ...
            current_total_weight += best_1for1_weight_change
        ...
        best_2for1_new_total_weight = -1 # Store new total weight directly
            ...
            for j in range(i + 1, len(temp_selected_item_info)):
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
                    ...
                    if potential_new_weight <= capacity and current_2for1_value_gain > best_2for1_value_gain:
                        ...
                        best_2for1_new_total_weight = potential_new_weight
        ...
        if best_2for1_value_gain > 0:
            ...
            current_total_weight = best_2for1_new_total_weight
