def heuristic(items_state):
    ...
    # --- Final Greedy Fill Pass ---
    # After local search stabilizes, try to fill any remaining capacity with unselected items
    # sorted by density (highest first) without removing anything.
    final_fill_items = list(unselected_items_dict.values())
    # Sort by density desc, then value desc (same as initial greedy sort)
    final_fill_items.sort(key=lambda x: (x[0], x[1]), reverse=True) 

    for density, value, weight, original_index in final_fill_items:
        if current_total_weight + weight <= capacity:
            current_selected_items_indices.add(original_index)
            current_total_weight += weight
            current_total_value += value
            # No need to remove from unselected_items_dict as this is the final step
            # and unselected_items_dict is not used after this.
