def heuristic(items_state):
    ...
    while improvement_made:
        ...
        temp_selected_item_info.sort(key=lambda x: (x[0], -x[1])) # IMPROVEMENT: Added -x[1] for weight desc
        ...
        temp_unselected_items_list_by_value.sort(key=lambda x: (x[1], x[0]), reverse=True) # IMPROVEMENT: Added x[0] for density desc
        ...
        temp_unselected_items_list_by_density.sort(key=lambda x: (x[0], x[1]), reverse=True) # IMPROVEMENT: Added x[1] for value desc
