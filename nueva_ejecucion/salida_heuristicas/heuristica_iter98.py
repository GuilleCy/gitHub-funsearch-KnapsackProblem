def heuristic(items_state):
    ...
    # --- Local Search: Iterative 0-weight, fill-up, 1-for-1, 1-for-k, and 2-for-1 Swaps ---
    ...
    while improvement_made:
        ...
        if zero_weight_items_added_this_pass:
            improvement_made = True
            # Do not 'continue' immediately, allow Phase 0.5 to run in the same iteration
            # as 0-weight items might free up space for other items.

        # --- NEW PHASE 0.5: Fill remaining capacity with unselected items (greedy) ---
        # This phase tries to add any unselected items that now fit,
        # especially after 0-weight items were added or if capacity was not fully used initially.
        fill_capacity_made_improvement = False
        
        # Re-sort unselected items by density for this greedy fill
        temp_unselected_for_fill = list(unselected_items_dict.values())
        temp_unselected_for_fill.sort(key=lambda x: (x[0], x[1]), reverse=True) # density desc, value desc

        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_for_fill:
            if current_total_weight + unsel_weight <= capacity:
                current_selected_items_indices.add(unsel_idx)
                current_total_weight += unsel_weight
                current_total_value += unsel_value
                del unselected_items_dict[unsel_idx]
                fill_capacity_made_improvement = True
        
        if fill_capacity_made_improvement:
            improvement_made = True
            continue # Restart the 'while' loop as state has changed significantly
        ...
        # This phase is executed only if Phase 0, Phase 0.5 and Phase 1 did not find any improvement in the current iteration.
        ...
        for sel_weight, sel_value, sel_idx in temp_selected_item_info_by_weight_desc: # Note order of (weight, value, idx)
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density:
                ...
                # Phase 0.5 ensures any simple fills are already done.
        ...
        # This phase is executed only if Phase 0, Phase 0.5, Phase 1 and Phase 2 did not find any improvement in the current iteration.
