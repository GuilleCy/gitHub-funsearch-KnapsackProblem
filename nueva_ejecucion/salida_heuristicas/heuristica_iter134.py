def heuristic(items_state):
    ...
    # --- Local Search: Iterative 0-weight, 1-for-1, 1-for-k, 2-for-1, and Greedy Add Swaps ---
    # This phase attempts to improve the solution by exploring different neighborhood types.
    ...
    while improvement_made:
        ...
        # Unselected items are sorted by density descending for greedy filling in 1-for-k and Phase 4.
        ...
        # --- Phase 4: Greedy Add non-zero weight items ---
        # This phase is executed only if previous phases (0, 1, 2, 3) did not find any improvement
        # in the current iteration. This attempts to add items that simply fit without removing anything.
        greedy_add_made_improvement = False
        # Iterate over unselected items sorted by density.
        # We need to use a temporary list and iterate safely while modifying the dictionary.
        # Rebuilding temp_unselected_items_list_by_density for a fresh view might be safer, 
        # but for performance, we use the already prepared one, knowing it's still consistent.
        # To avoid issues with changing dict during iteration, process items that are to be added.
        items_to_add_this_pass = []
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_density:
            # Only consider items with positive weight, as 0-weight items are handled in Phase 0.
            if unsel_weight > 0 and current_total_weight + unsel_weight <= capacity:
                items_to_add_this_pass.append((unsel_value, unsel_weight, unsel_idx))
        
        # Now add them and update state
        for unsel_value, unsel_weight, unsel_idx in items_to_add_this_pass:
            current_selected_items_indices.add(unsel_idx)
            current_total_weight += unsel_weight
            current_total_value += unsel_value
            del unselected_items_dict[unsel_idx] # Remove from unselected_items_dict
            greedy_add_made_improvement = True # Indicate that an improvement was made

        if greedy_add_made_improvement:
            improvement_made = True
            continue # Restart outer 'while' loop (to allow new swaps based on added items)

    # If improvement_made is True here, the outer while loop will restart.
    # If it's still False after all four phases, the while loop terminates.
