def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # This list is only needed here, so create it conditionally.
            ...
            for j in range(i + 1, len(temp_selected_item_info)):
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
                    ...
                    if potential_new_weight <= capacity and potential_new_value > current_total_value:
                        ...
                        sel1_density = values[sel1_idx] / weights[sel1_idx] if weights[sel1_idx] > 0 else (float('inf') if values[sel1_idx] > 0 else 0.0)
                        unselected_items_dict[sel1_idx] = (sel1_density, values[sel1_idx], weights[sel1_idx], sel1_idx)
                        
                        sel2_density = values[sel2_idx] / weights[sel2_idx] if weights[sel2_idx] > 0 else (float('inf') if values[sel2_idx] > 0 else 0.0)
                        unselected_items_dict[sel2_idx] = (sel2_density, values[sel2_idx], weights[sel2_idx], sel2_idx)
