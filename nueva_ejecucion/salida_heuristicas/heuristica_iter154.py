def heuristic(items_state):
    ...
    # --- Local Search: Iterative 0-weight, 1-for-1, 1-for-k, 2-for-1 Swaps, and Greedy Fill ---
    ...
    while improvement_made:
        ...
        # Using temp_selected_item_info_by_weight_desc to prioritize removing larger items.
        for sel_weight, sel_value, sel_idx in temp_selected_item_info_by_weight_desc: 
        ...
        # --- Phase 4: Greedy Fill Remaining Capacity ---
        # This phase tries to fill any remaining capacity with unselected items.
        # It's run last in an iteration to ensure the knapsack is as full as possible
        # after all swap attempts.
        
        local_greedy_fill_made = False
        # Iterate over a copy of the list to allow modification of unselected_items_dict
        # temp_unselected_items_list_by_density is already sorted by density (desc) and value (desc)
        for unsel_density, unsel_value, unsel_weight, unsel_idx in list(temp_unselected_items_list_by_density):
            # Check if item is still unselected (it should be, as continue would have restarted if a swap occurred).
            if unsel_idx in unselected_items_dict: 
                if current_total_weight + unsel_weight <= capacity:
                    current_selected_items_indices.add(unsel_idx)
                    current_total_weight += unsel_weight
                    current_total_value += unsel_value
                    del unselected_items_dict[unsel_idx]
                    local_greedy_fill_made = True
        
        if local_greedy_fill_made:
            improvement_made = True
            # No continue here. This is the last phase. If it improved, improvement_made is True,
            # and the outer while loop will naturally restart.
