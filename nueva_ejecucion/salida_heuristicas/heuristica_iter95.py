def heuristic(items_state):
    ...
    # --- Final Greedy Fill for Remaining Capacity ---
    # After local search converges, there might still be some capacity left.
    # Try to fill it with remaining unselected items, sorted by density.
    remaining_capacity = capacity - current_total_weight
    if remaining_capacity > 0 and unselected_items_dict:
        final_fill_candidates = list(unselected_items_dict.values())
        # Sort by density descending, then value descending (consistent with initial greedy)
        final_fill_candidates.sort(key=lambda x: (x[0], x[1]), reverse=True) 

        for unsel_density, unsel_value, unsel_weight, unsel_idx in final_fill_candidates:
            if unsel_weight <= remaining_capacity:
                current_selected_items_indices.add(unsel_idx)
                current_total_weight += unsel_weight
                current_total_value += unsel_value
                remaining_capacity -= unsel_weight
                # Optionally remove from unselected_items_dict for conceptual cleanliness,
                # though not strictly necessary as it's not used after this.
                # del unselected_items_dict[unsel_idx] 
