def heuristic(items_state):
    ...
    # IMPROVEMENT: Use a dictionary for unselected items for efficient O(1) add/remove by index.
    ...
    # --- Post-optimization Fill-up ---
    # After all swap attempts are exhausted, try to fill any remaining capacity
    # with unselected items, prioritized by density.
    
    # Convert unselected items dict to a list for sorting
    # This list needs to be created from the current state of unselected_items_dict
    remaining_unselected_items = list(unselected_items_dict.values())
    remaining_unselected_items.sort(key=lambda x: x[0], reverse=True) # Sort by density descending

    for density, value, weight, original_index in remaining_unselected_items:
        # Check if the item is still truly unselected and if it fits.
        # The 'original_index in unselected_items_dict' check is crucial here,
        # as the dict might have been modified by previous steps.
        if original_index in unselected_items_dict:
            # Handle 0-weight items with positive value first, as they always fit.
            if weight == 0 and value > 0:
                current_selected_items_indices.add(original_index)
                current_total_value += value
                del unselected_items_dict[original_index] 
                # No change to current_total_weight for 0-weight items
            elif current_total_weight + weight <= capacity:
                current_selected_items_indices.add(original_index)
                current_total_weight += weight
                current_total_value += value
                del unselected_items_dict[original_index] 
