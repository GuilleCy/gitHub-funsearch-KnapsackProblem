def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # This list is created here as a snapshot for Phase 2.
        temp_unselected_items_list_by_density_for_phase2 = list(unselected_items_dict.values())
        temp_unselected_items_list_by_density_for_phase2.sort(key=lambda x: x[0], reverse=True) 
        ...
        for sel_value, sel_weight, sel_idx in temp_selected_item_info:
            ...
            # Greedily fill available_capacity with unselected items using the density-sorted snapshot
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density_for_phase2:
        ...
        # If it's still False after all three phases, we proceed to the tightening pass.

        # --- Phase 4: Tightening Pass (greedy add of remaining items) ---
        # This phase is executed only if no improvements were found in Phases 1-3.
        # It tries to add any remaining unselected items that now fit, due to
        # capacity being freed up by swaps or 0-weight items being added.
        if not improvement_made: # Only run if no improvement from Phases 1-3
            # Create a fresh list of currently unselected items, sorted by density,
            # as unselected_items_dict might have changed during previous phases.
            current_unselected_by_density = []
            for original_index, item_tuple in unselected_items_dict.items():
                current_unselected_by_density.append(item_tuple)
            current_unselected_by_density.sort(key=lambda x: x[0], reverse=True)

            for unsel_density, unsel_value, unsel_weight, unsel_idx in current_unselected_by_density:
                # Optimization: 0-weight items should always be added if they provide value
                if unsel_weight == 0 and unsel_value > 0:
                    current_selected_items_indices.add(unsel_idx)
                    current_total_value += unsel_value
                    del unselected_items_dict[unsel_idx] # Remove from unselected
                    improvement_made = True # An improvement was made, outer loop will restart.
                    continue # Don't consume capacity, continue to next item
                ...
                if current_total_weight + unsel_weight <= capacity:
                    ...
                    del unselected_items_dict[unsel_idx] # Remove from unselected
                    improvement_made = True # An improvement was made, outer loop will restart.
                    # Do NOT break here. Continue trying to add more items greedily to fill remaining space.
        # If improvement_made is True here, the outer while loop will restart.
