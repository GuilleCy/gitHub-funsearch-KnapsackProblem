def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # NEW: Selected items for 1-for-k: sorted by weight descending (candidates for removal).
        ...
        for idx in current_selected_items_indices:
            temp_selected_item_info_by_weight_desc.append((values[idx], weights[idx], idx)) # Store (value, weight, idx)
        temp_selected_item_info_by_weight_desc.sort(key=lambda x: (-x[1], x[0])) # Sort by weight DESC (x[1]), then value ASC (x[0])
        ...
        # Using temp_selected_item_info_by_weight_desc to prioritize freeing up more capacity
        for sel_value, sel_weight, sel_idx in temp_selected_item_info_by_weight_desc: 
    ...
    # --- Post-loop Greedy Fill ---
    # After no more swaps improve the solution, try to fill any remaining capacity
    # with unselected items (sorted by density).
    # Re-sort unselected items by density in case the dictionary order changed or
    # new items were added to it during the last iteration.
    final_unselected_items_list_by_density = list(unselected_items_dict.values())
    final_unselected_items_list_by_density.sort(key=lambda x: (x[0], x[1]), reverse=True)

    for unsel_density, unsel_value, unsel_weight, unsel_idx in final_unselected_items_list_by_density:
        if current_total_weight + unsel_weight <= capacity:
            current_selected_items_indices.add(unsel_idx)
            current_total_weight += unsel_weight
            current_total_value += unsel_value
            # No need to remove from unselected_items_dict here, as this is the final step.
