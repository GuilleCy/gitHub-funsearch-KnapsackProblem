def heuristic(items_state):
    ...
    # --- Local Search: Iterative 0-weight, 1-for-1, 1-for-k, 2-for-1, and Add-Only Swaps ---
    ...
    while improvement_made:
        ...
        # --- Phase 4: Add any fitting unselected items ---
        # This phase is executed only if Phase 0, 1, 2, and 3 did not find any improvement.
        # It attempts to fill any remaining capacity with unselected items without removing any.
        
        current_capacity_remaining = capacity - current_total_weight
        
        # Iterate through unselected items by density (best candidates for adding)
        # Use a copy of the list to allow modification of unselected_items_dict during iteration
        for unsel_density, unsel_value, unsel_weight, unsel_idx in list(temp_unselected_items_list_by_density):
            # Ensure the item is still unselected (might have been added by a previous phase if continue was missed)
            if unsel_idx in unselected_items_dict and unsel_weight <= current_capacity_remaining:
                current_selected_items_indices.add(unsel_idx)
                current_total_weight += unsel_weight
                current_total_value += unsel_value
                current_capacity_remaining -= unsel_weight # Update remaining capacity
                del unselected_items_dict[unsel_idx]
                improvement_made = True # An item was added
        
        if improvement_made:
            continue # Restart outer 'while' loop if items were added
