def heuristic(items_state):
    ...
    while improvement_made:
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            ...
            # Pruning: If an unselected item is too heavy to fit into the remaining capacity, skip it.
            if unsel_weight > capacity - current_total_weight:
                continue
    ...
    # --- Final Fill-up Pass (after local search stabilizes) ---
    # Try to add any remaining unselected items that fit into the current empty space.
    # This can catch items that were too small to be part of a profitable swap,
    # but now fit due to removed items or a less full knapsack.
    
    # Sort remaining unselected items by density (and value for tie-breaking)
    final_unselected_items = list(unselected_items_dict.values())
    final_unselected_items.sort(key=lambda x: (x[0], x[1]), reverse=True)

    for unsel_density, unsel_value, unsel_weight, unsel_idx in final_unselected_items:
        # Safeguard: ensure item is not already selected (e.g., if unselected_items_dict was not perfectly consistent)
        if unsel_idx in current_selected_items_indices: 
            continue
        
        # Add 0-weight, positive-value items directly
        if unsel_weight == 0 and unsel_value > 0:
            current_selected_items_indices.add(unsel_idx)
            current_total_value += unsel_value
            continue
            
        # Add items if they fit into the remaining capacity
