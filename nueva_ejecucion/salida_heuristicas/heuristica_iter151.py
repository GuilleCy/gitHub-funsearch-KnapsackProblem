def heuristic(items_state):
    ...
    while improvement_made:
        ...
        for idx in current_selected_items_indices:
            temp_selected_item_info_by_weight_desc.append((weights[idx], values[idx], idx)) # Store (weight, value, idx)
        ...
        temp_unselected_items_list_by_value.sort(key=lambda x: (x[1], x[0]), reverse=True) # x[1] is value, x[0] is density
        ...
        temp_unselected_items_list_by_density.sort(key=lambda x: (x[0], x[1]), reverse=True) # x[0] is density, x[1] is value
        ...
        for sel_weight, sel_value, sel_idx in temp_selected_item_info_by_weight_desc: # Note order of (weight, value, idx)
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density:
                # After Phase 0, all 0-weight, positive-value items should already be selected.
                # So we don't need a special check for them here.
