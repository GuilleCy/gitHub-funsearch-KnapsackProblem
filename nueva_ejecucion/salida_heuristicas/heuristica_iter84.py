def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # --- PHASE 0.5: Fill remaining capacity with highest density unselected items ---
        # This helps to improve knapsack filling by adding items that simply fit.
        residual_fill_made = False
        remaining_capacity = capacity - current_total_weight
        
        if remaining_capacity > 0 and unselected_items_dict:
            # Create a temporary sorted list specifically for this phase
            temp_unselected_for_residual_fill = list(unselected_items_dict.values())
            temp_unselected_for_residual_fill.sort(key=lambda x: x[0], reverse=True) # Sort by density desc

            for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_for_residual_fill:
                if unsel_weight <= remaining_capacity:
                    ...
                    del unselected_items_dict[unsel_idx]
                    residual_fill_made = True
        
        if residual_fill_made:
            improvement_made = True
        ...
        # This phase is executed only if Phase 0 and Phase 0.5 and Phase 1 did not find any improvement in the current iteration.
