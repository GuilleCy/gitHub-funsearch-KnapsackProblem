def heuristic(items_state):
    ...
    # The original code used a list, which required O(N) list comprehensions for updates.
    ...
    while improvement_made:
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            ...
            # This is necessary because temp_unselected_items_list_by_value is a snapshot.
            ...
            if unsel_weight == 0 and unsel_value > 0:
                ...
                del unselected_items_dict[unsel_idx] # O(1) removal from dictionary.
                improvement_made = True
        ...
        # This phase is more computationally intensive, so only run if 1-for-1 didn't find anything.
        if not improvement_made:
            ...
            # Iterate through selected items as candidates for removal
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                # Skip if this item was already unselected in a previous swap within this 'while' iteration.
                if sel_idx not in current_selected_items_indices:
                    continue

                # Temporarily remove sel_idx to calculate available space
                ...
                # The goal is to maximize value in the freed space.
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_density:
                    ...
                    # The unsel_idx in current_selected_items_indices check is important
