def heuristic(items_state):
    ...
    while improvement_made:
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            ...
            # These items don't consume capacity and always improve value if positive.
            if unsel_weight == 0 and unsel_value > 0:
                ...
                improvement_made = True
                break # Break from current loop to restart the 'while' loop to re-evaluate state
        ...
        # This phase is executed only if the previous 1-for-1 pass found no improvements.
        if not improvement_made:
            ...
            # Iterate through selected items (candidates for removal to make space)
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                # Skip if this item was already unselected in a previous swap within this 'while' iteration.
                if sel_idx not in current_selected_items_indices:
                    continue

                # Temporarily remove sel_idx to see how much capacity becomes available
                ...
                # Greedily fill the available_capacity with unselected items (by density)
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_density:
                    # Skip if already selected or if it's the item just considered for removal (sel_idx)
                    if unsel_idx in current_selected_items_indices or unsel_idx == sel_idx:
                        continue
                    
                    # 0-weight items with positive value should always be added if not already
                    if unsel_weight == 0 and unsel_value > 0:
                ...
                # The primary condition for an improvement is an increase in total value.
                if temp_current_total_value + potential_add_value > current_total_value:
                    ...
                    for add_idx in potential_add_indices:
                        ...
                        del unselected_items_dict[add_idx] # Remove from unselected_items_dict
