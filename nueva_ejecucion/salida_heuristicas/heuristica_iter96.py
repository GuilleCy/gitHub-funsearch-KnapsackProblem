def heuristic(items_state):
    ...
    # --- Local Search: Iterative 0-weight, 1-for-1, 1-for-k, and 2-for-1 Swaps ---
    ...
    while improvement_made:
        ...
        # Selected items for 1-for-1 and 2-for-1: sorted by value ascending (candidates for removal).
        # Added secondary sort by weight ascending to free up more capacity if values are equal.
        temp_selected_item_info_by_value_asc = []
        for idx in current_selected_items_indices:
            temp_selected_item_info_by_value_asc.append((values[idx], weights[idx], idx))
        temp_selected_item_info_by_value_asc.sort(key=lambda x: (x[0], x[1])) 

        # Selected items for 1-for-k: sorted by weight descending (candidates for removal).
        # Added secondary sort by value ascending to prefer removing less valuable items if weights are equal.
        temp_selected_item_info_by_weight_desc = []
        for idx in current_selected_items_indices:
            temp_selected_item_info_by_weight_desc.append((weights[idx], values[idx], idx))
        temp_selected_item_info_by_weight_desc.sort(key=lambda x: (-x[0], x[1])) # Sort by weight DESC, then value ASC
        ...
        # Initialize variables to track the best swap found across all phases in this iteration
        best_overall_value_gain = 0
        best_overall_swap_type = None # '1-for-1', '1-for-k', '2-for-1'
        best_overall_swap_details = None # tuple of (sel_idx(s), unsel_idx(s), add_weight, add_value)
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            ...
            for sel_value, sel_weight, sel_idx in temp_selected_item_info_by_value_asc:
        ...
        if best_1for1_value_gain > best_overall_value_gain:
            best_overall_value_gain = best_1for1_value_gain
            best_overall_swap_type = '1-for-1'
            best_overall_swap_details = (best_1for1_sel_idx, best_1for1_unsel_idx)
        ...
        # Using temp_selected_item_info_by_weight_desc to free up more capacity
        for sel_weight, sel_value, sel_idx in temp_selected_item_info_by_weight_desc: # Note order of (weight, value, idx)
            ...
            for unsel_density, unsel_value, unsel_weight, unsel_idx_to_add in temp_unselected_items_list_by_density:
                # Items in unselected_items_dict are guaranteed to be unselected and not sel_idx.
                # Phase 0 ensures all 0-weight, positive-value items are already selected.
            ...
            # Only consider swaps that add items and provide a gain
        ...
        if best_1fork_value_gain > best_overall_value_gain:
            best_overall_value_gain = best_1fork_value_gain
            best_overall_swap_type = '1-for-k'
            best_overall_swap_details = (best_1fork_sel_idx, best_1fork_add_indices, best_1fork_add_weight, best_1fork_add_value)
        ...
        for i in range(len(temp_selected_item_info_by_value_asc)):
            sel1_value, sel1_weight, sel1_idx = temp_selected_item_info_by_value_asc[i]

            for j in range(i + 1, len(temp_selected_item_info_by_value_asc)):
                sel2_value, sel2_weight, sel2_idx = temp_selected_item_info_by_value_asc[j]
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
                    ...
                    # Items in unselected_items_dict are guaranteed to be unselected and not sel1_idx or sel2_idx.
        ...
        if best_2for1_value_gain > best_overall_value_gain:
            best_overall_value_gain = best_2for1_value_gain
            best_overall_swap_type = '2-for-1'
            best_overall_swap_details = (best_2for1_sel1_idx, best_2for1_sel2_idx, best_2for1_unsel_idx)

        # --- Apply the best overall swap found in this iteration ---
        if best_overall_value_gain > 0:
            improvement_made = True
            if best_overall_swap_type == '1-for-1':
                sel_idx, unsel_idx = best_overall_swap_details
                current_selected_items_indices.remove(sel_idx)
                current_selected_items_indices.add(unsel_idx)
                current_total_weight += (weights[unsel_idx] - weights[sel_idx])
                current_total_value += best_overall_value_gain
                del unselected_items_dict[unsel_idx] 
                s_density, s_value, s_weight = item_properties[sel_idx]
                unselected_items_dict[sel_idx] = (s_density, s_value, s_weight, sel_idx)
            elif best_overall_swap_type == '1-for-k':
                sel_idx, add_indices, add_weight, add_value = best_overall_swap_details
                current_selected_items_indices.remove(sel_idx)
                ...
                s_density, s_value, s_weight = item_properties[sel_idx]
                unselected_items_dict[sel_idx] = (s_density, s_value, s_weight, sel_idx)
                current_total_value = (current_total_value - values[sel_idx]) + add_value
                current_total_weight = (current_total_weight - weights[sel_idx]) + add_weight
            elif best_overall_swap_type == '2-for-1':
                sel1_idx, sel2_idx, unsel_idx = best_overall_swap_details
                current_selected_items_indices.remove(sel1_idx)
                current_selected_items_indices.remove(sel2_idx)
                current_selected_items_indices.add(unsel_idx)
                current_total_weight = current_total_weight - weights[sel1_idx] - weights[sel2_idx] + weights[unsel_idx]
                current_total_value += best_overall_value_gain
                del unselected_items_dict[unsel_idx]
                s1_density, s1_val, s1_w = item_properties[sel1_idx]
                unselected_items_dict[sel1_idx] = (s1_density, s1_val, s1_w, sel1_idx)
                s2_density, s2_val, s2_w = item_properties[sel2_idx]
                unselected_items_dict[sel2_idx] = (s2_density, s2_val, s2_w, sel2_idx)
            # No 'continue' here, the outer while loop will naturally re-evaluate improvement_made
