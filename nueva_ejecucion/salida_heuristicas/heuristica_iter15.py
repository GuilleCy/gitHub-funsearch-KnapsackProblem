def heuristic(items_state):
    ...
    while improvement_made:
        ...
        # This is built from current_selected_items_indices, so it's fresh each iteration.
        ...
        # This is built from unselected_items_dict, so it's fresh each iteration.
        ...
        for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_value:
            ...
            # This check is crucial because temp_unselected_items_list_by_value is a snapshot.
            ...
            if unsel_weight == 0 and unsel_value > 0:
                ...
                del unselected_items_dict[unsel_idx] # O(1) removal
                improvement_made = True
            ...
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                ...
                if current_total_weight + weight_change <= capacity and value_change > 0:
                    ...
                    current_total_value += value_change # Fixed typo: was 'value_value'
                    ...
                    unselected_items_dict[sel_idx] = (sel_item_density, values[sel_idx], weights[sel_idx], sel_idx) # O(1) addition
        ...
        # This phase is more computationally intensive, so we only run it if 1-for-1 swaps
        # didn't yield any improvement in the current iteration.
        if not improvement_made:
            ...
            # Iterate through selected items (candidates for removal)
            for sel_value, sel_weight, sel_idx in temp_selected_item_info:
                # Skip if this item was already unselected in a previous swap within this 'while' iteration.
                if sel_idx not in current_selected_items_indices:
                    continue

                # Temporarily remove sel_idx to calculate available capacity
                ...
                for unsel_density, unsel_value, unsel_weight, unsel_idx in temp_unselected_items_list_by_density:
                    ...
                    if unsel_idx in current_selected_items_indices or unsel_idx == sel_idx:
                        continue
                    
                    # Optimization: 0-weight items should always be added if they provide value
                    if unsel_weight == 0 and unsel_value > 0:
                        potential_add_indices.add(unsel_idx)
                ...
                # The primary check is if the total value increases, and that we actually added items.
                if potential_add_indices and (temp_current_total_value + potential_add_value > current_total_value):
                    ...
                    for add_idx in potential_add_indices:
                        ...
                        # O(1) removal from unselected_items_dict
                    ...
                    improvement_made = True
                    break # Break from inner loop (selected items) to restart the while loop
            # If improvement was made in 1-for-k, break from the outer while loop's current iteration
            if improvement_made:
                break 
